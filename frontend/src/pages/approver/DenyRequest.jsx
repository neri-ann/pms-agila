...
export default function RejectConfirmationModal({ open, setOpen, request, onRejectSuccess }) {
  ...
  const handleReject = () => {
    ...
    // Option 1: Update status to "Rejected" (if you want to keep the record)
    const rejectionData = { status: "Rejected" };
    
    axios
      .put(`http://localhost:8000/approvalReqest/updateStatus/${request._id}`, rejectionData)
      .then(() => {
        setLoading(false);
    ...
    // Option 2: Delete the request (uncomment if you want to delete instead)
    /*
    axios
      .delete(`http://localhost:8000/procReqest/deleteRequest/${request.requestId}`)
      .then(() => {
        setLoading(false);
            ...
            >
              <Dialog.Panel className="relative transform overflow-hidden rounded-xl bg-white text-left shadow-2xl transition-all sm:my-8 sm:w-full sm:max-w-lg border border-gray-200">
                ...
                <DialogBody divider className="grid place-items-center px-6 py-4">
                  ...
                  <div className="text-center text-sm text-gray-600 bg-gray-50 p-3 rounded-lg border border-gray-200 mb-4">
                    <p><strong>Department:</strong> {request.department}</p>
                    <p><strong>Requestor:</strong> {request.faculty}</p>
                  </div>

                  <Typography
                    className="text-center font-normal text-sm px-4"
                    color="red"
                  >
                    <p className="text-red-600 bg-red-50 p-3 rounded-lg border border-red-200">
                      <strong>Warning:</strong> Rejecting this request will change its status to "Rejected" and the requestor will be notified of the rejection.
                    </p>
                  </Typography>
                </DialogBody>
